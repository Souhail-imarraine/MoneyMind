...
<div id="editSalaryModal" class="fixed inset-0 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-lg shadow-lg p-6 w-1/3 max-w-md mx-auto">
        ...
        <form id="editSalaryForm" method="POST" action="{{ route('dashboard.updateSalary', $user->id) }}">
            @csrf
            @method('PUT')
            <div class="form-group mb-4">
                <label for="salary" class="block text-sm font-medium text-gray-700">Nouveau Salaire</label>
                <input type="number" value="{{$user->salary}}" name="salary" id="salary" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring h-10" required>
                @error('salary')
